import Progress0 from "../assets/ProgressImage/Progress0.jpg";
import Progress1 from "../assets/ProgressImage/Progress1.jpg";
import Progress2 from "../assets/ProgressImage/Progress2.jpg";
import Progress3 from "../assets/ProgressImage/Progress3.jpg";
import Progress4 from "../assets/ProgressImage/Progress4.jpg";
import Progress5 from "../assets/ProgressImage/Progress5.jpg";

export const ProgressImage = [{ id: "0", name: "Progress0", src: Progress0 }];
export const contents = [
  {
    title: "OO시 차들이 홍수로 침수되었어요",
    organization: "사회지원재단",
    image: Progress0,
    progress: 21,
    barWidth: "90px",
    totalDonation: "1,050,000",
    targetDonation: "5,000,000",
    categories: ["all", "disaster"],
    date: "2023년",
    manager:"김수민",
    address: "0x2401bC67706121CCa309d6cd01a05E2fdF9Cff4a",
    text: `저희 지역은 최근 폭우로 인한 피해로 큰 고통을 겪고 있습니다. 
           많은 가정들이 물에 잠기고, 소중한 물건들을 잃어버리며 집을 잃게 되었습니다. 그중 하나가 바로 저희 가정입니다.
           이 글을 쓰는 순간에도 많은 가정들이 위급한 상황에 처해있습니다. 
           그래서 저희는 여러분들의 따뜻한 마음과 지원을 필요로 합니다. 이 힘든 시기에 도움을 주시기를 간곡히 부탁드립니다.`,
            comments: [
              {
                userId: "익명",
                date: "2023.08.02",
                comment: "응원합니다!!",
              },
              {
                userId: "익명",
                date: "2023.08.02",
                comment: "폭우로 너무 많은 사람들이 피해를 입어서 마음 아프네요",
              },
              {
                userId: "익명",
                date: "2023.08.01",
                comment: "응원합니다!!",
              },
              {
                userId: "익명",
                date: "2023.08.01",
                comment: "슬프네요..",
              },
              {
                userId: "익명",
                date: "2023.08.01",
                comment: "하루빨리 피해복구가 됐으면 좋겠습니다~",
              },
              {
                userId: "익명",
                date: "2023.07.31",
                comment: "큰 피해가 없었으면합니다!!",
              },
              {
                userId: "익명",
                date: "2023.07.31",
                comment: "응원합니다!!",
              },
            ],
          },

  {
    title: "결식 아동은 도시락이 필요해요",
    organization: "서울시희망조합",
    image: Progress1,
    progress: 32,
    barWidth: "110px",
    totalDonation: "640,000",
    targetDonation: "2,000,000",
    categories: ["all", "child"],
    date: "2023년",
    manager:"박정우",
    address: "0x2401bC67706121CCa309d6cd01a05E2fdF9Cff4a",

    text: `규민이는 아무런 잘못이 없음에도 불구하고, 가난과 궁핍함으로 인해 매일매일 배고픔과 싸워야 합니다. 
           아침이면 한 끼의 식사를 위해 눈을 뜨지만, 라면이나 인스턴트 식품으로 배를 채울 수 밖에 없습니다. 
           그럼에도 불구하고 규민이는 포기하지 않고 용기와 희망을 가지고 살아가고 있습니다. 
           이번 방학에도 급식 사각지대 없이 모든 아동이 따뜻한 보살핌 속에서 방학을 보낼 수 있게, 눈치 보면서 밥 먹는 아이들이 없도록 도시락을 선물해주세요! `,
           comments: [
            {
              userId: "익명",
              date: "2023.08.02",
              comment: "응원합니다!!",
            },
            {
              userId: "익명",
              date: "2023.08.02",
              comment: "아이고 규민아ㅜㅜ",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "응원합니다!!",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "슬프네요..",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "잘 챙겨먹고 기운내길",
            },
            {
              userId: "익명",
              date: "2023.07.31",
              comment: "응원해요..",
            },
            {
              userId: "익명",
              date: "2023.07.31",
              comment: "응원합니다!!",
            },
          ],
  },
  {
    title: "여름나기가 두려운 어르신들",
    organization: "사회지원재단",
    image: Progress2,
    progress: 75,
    barWidth: "230px",
    totalDonation: "750,000",
    targetDonation: "1,000,000",
    categories: ["all", "old"],
    date: "2023년",
    manager:"이서윤",
    address: "0x2401bC67706121CCa309d6cd01a05E2fdF9Cff4a",

    text: `올해도 어김없이 뜨거운 여름철이 찾아왔습니다. 특히 우리 어르신들은 더위로 인해 힘들어하고 계실 것입니다. 
           '사랑의 선풍기 기부 캠페인'을 시작합니다. 
           이번 캠페인은 우리 지역 어르신들에게 선풍기를 지원하여 더위를 덜어주는 작은 도움을 드리고자 합니다.
           여러분의 따뜻한 마음과 기부로 우리 어르신들의 여름을 쾌적하게 만들어주세요.`,
           comments: [
            {
              userId: "익명",
              date: "2023.08.02",
              comment: "더운 여름 잘 이겨내세요!!",
            },
            {
              userId: "익명",
              date: "2023.08.02",
              comment: "마음 아프네요ㅜ",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "응원합니다!!",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "슬프네요..",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "어르신들이 건강하게 여름을 나기를",
            },
            {
              userId: "익명",
              date: "2023.07.31",
              comment: "항상 응원해요.",
            },
            {
              userId: "익명",
              date: "2023.07.31",
              comment: "응원합니다!!",
            },
          ], },
  {
    title: "치료가 필요한 정우를 도와주세요",
    organization: "굿네이버스",
    image: Progress3,
    progress: 25,
    barWidth: "110px",
    totalDonation: "750,000",
    targetDonation: "3,000,000",
    categories: ["all", "child", "medical"],
    date: "2023년",
    manager:"김도윤",
    address: "0x2401bC67706121CCa309d6cd01a05E2fdF9Cff4a",

    text: `정우는 지난 몇 달 동안 건강 상태가 더욱 악화되었습니다. 정우는 소아암 환아로 치료 비용은 상당히 부담스러운 상황입니다. 
           이런 어려운 시기에 우리의 도움이 정우에게 큰 응원이 될 수 있습니다.
           하지만 우리는 혼자서는 모든 것을 할 수 없습니다. 여러분의 따뜻한 마음과 아무리 작은 기부라도 큰 도움이 될 것입니다.
           여러분의 기부금은 진료 비용을 지원하고 치료에 대한 접근성을 향상시키는 데 사용될 것입니다. 
           여러분 한분한분의 관심과 선한 마음이 얼마나 큰 힘이 될 것입니다.`,
           comments: [
            {
              userId: "익명",
              date: "2023.08.02",
              comment: "응원합니다!!",
            },
            {
              userId: "익명",
              date: "2023.08.02",
              comment: "정우야 힘내..",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "응원합니다!!",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "아이구ㅜ",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "치료 비용에 힘이 힘이 되었으면",
            },
            {
              userId: "익명",
              date: "2023.07.31",
              comment: "응원해요.",
            },
            {
              userId: "익명",
              date: "2023.07.31",
              comment: "응원합니다!!",
            },
          ], },
  {
    title: "장애인",
    organization: "장애인복지관",
    image: Progress4,
    progress: 33,
    barWidth: "125px",
    totalDonation: "330,000",
    targetDonation: "1,000,000",
    categories: ["all", "disabled"],
    date: "2023년",
    manager:"서민기",
    address: "0x2401bC67706121CCa309d6cd01a05E2fdF9Cff4a",

    text: `미세먼지로 인한 환경 오염은 우리가 직접적으로 개입할 수 있는 문제이기도 합니다. 
           우리의 작은 노력이 모여 큰 변화를 만들 수 있습니다. 
           그렇기에 저희는 여러분들의 따뜻한 마음과 도움을 기다리고 있습니다.
           저희 지역의 기관과 단체들은 미세먼지 대책을 위해 다양한 활동을 전개하고 있습니다. 
           그러나 이러한 활동들은 여러분들의 기부와 참여가 없이는 한계가 있습니다. 
           우리 지역 사회가 함께 모여 더 나은 환경을 만들어가기 위해서는 여러분들의 지원이 필요합니다.`,
           comments: [
            {
              userId: "익명",
              date: "2023.08.02",
              comment: "응원합니다!!",
            },
            {
              userId: "익명",
              date: "2023.08.02",
              comment: "미세먼지 너무 심해요",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "응원합니다!!",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "멋져요",
            },
            {
              userId: "익명",
              date: "2023.08.01",
              comment: "맑은 공기가 그리워요ㅠㅠ",
            },
            {
              userId: "익명",
              date: "2023.07.31",
              comment: "마스크 그만..",
            },
            {
              userId: "익명",
              date: "2023.07.31",
              comment: "응원합니다!!",
            },
          ], },

          {
            title: "어르신",
            organization: "노인지원센터",
            manager:"김동국",
            image: Progress5,
            progress: 70,
            barWidth: "210px",
            totalDonation: "1,400,000",
            targetDonation: "2,000,000",
            categories: ["all", "old"],
            date: "2023년",
            text: `우리 사랑하는 반려견 코코가 응급한 상태에 처해있습니다. 
                   코코는 우리 가족의 소중한 구성원으로서 항상 행복한 시간을 함께 나누었습니다. 
                   하지만 현재 건강이 위태롭고, 의료 비용이 절실한 상황입니다.
                   의료 비용이 부담스러운 만큼, 우리는 여러분들의 따뜻한 마음과 도움을 빌리고자 합니다. 
                   코코의 수술을 위해 기부를 모으고 있으며, 작은 기부라도 큰 도움이 될 것입니다. 
                   여러분들의 선행에 감사드립니다.`,
                   comments: [
                   {
                    userId: "익명",
                    date: "2023.08.02",
                    comment: "코코 응원합니다!!",
                  },
                  {
                    userId: "익명",
                    date: "2023.08.02",
                    comment: "귀여운 코코ㅜㅜ",
                  },
                  {
                    userId: "익명",
                    date: "2023.08.01",
                    comment: "응원합니다!!",
                  },
                  {
                    userId: "익명",
                    date: "2023.08.01",
                    comment: "기운내길!!",
                  },
                  {
                    userId: "익명",
                    date: "2023.08.01",
                    comment: "코코야 잘 이겨내렴",
                  },
                  {
                    userId: "익명",
                    date: "2023.07.31",
                    comment: "응원합니다.",
                  },
                  {
                    userId: "익명",
                    date: "2023.07.31",
                    comment: "응원합니다!!",
                  },
                ], }
              ];

export const List = [
  {
    userId: "공소연",
    date: "2023.7.30",
    howMany: "236,159",
  },
  {
    userId: "박지형",
    date: "2023.7.29",
    howMany: "410,000",
  },
  {
    userId: "밥먹는 제이지",
    date: "2023.7.29",
    howMany: "348,000",
  },
  {
    userId: "2PlusOne",
    date: "2023.7.28",
    howMany: "1,230,000",
  },
  {
    userId: "정민희",
    date: "2023.7.26",
    howMany: "30,000",
  },
  {
    userId: "비공개",
    date: "2023.7.24",
    howMany: "480,000",
  },
  {
    userId: "박새로이",
    date: "2023.7.24",
    howMany: "300,000",
  },
];
